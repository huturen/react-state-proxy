...
```tsx
// 方式 1: `react-state-proxy` 提供了一个可选的方法来管理的异步状态。
...
type AsyncState = {
  ...
  resolved: any; // 如果asyncFunction执行成功，这个值将被设置成 asyncFunction 返回的值
  rejected: any; // 如果asyncFunction执行失败，这个值将被设置成 抛出的异常错误信息
  valueOf: Function;
};
```
